...
import threading
from mesh import *
script_path = "${HOME}/nrc_pkg/script/"
...
def copyConf():
    os.system("sudo ${HOME}/nrc_pkg/sw/firmware/copy " + str(model) + " " + strBDName())
    os.system("${HOME}/nrc_pkg/script/conf/etc/ip_config.sh " + strSTA() + " " +  str(relay_type) + " " + str(static_ip) + " " + str(batman))
...
def run_common():
    if int(max_cpuclock) == 1:
        ...
        os.system("sudo ${HOME}/nrc_pkg/script/conf/etc/clock_config.sh")
    ...
    print("sudo insmod ${HOME}/nrc_pkg/sw/driver/nrc.ko " + insmod_arg)
    os.system("sudo insmod ${HOME}/nrc_pkg/sw/driver/nrc.ko " + insmod_arg + "")
    ...
    os.system('${HOME}/nrc_pkg/script/cli_app set txpwr limit ' + str(max_txpwr))
    if strSTA() != 'SNIFFER':
    ...
    os.system('${HOME}/nrc_pkg/script/cli_app set gi ' + guard_int)
...
def run_ibss(interface):
    ...
    if strSecurity() == 'OPEN':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/ibss_halow_open.conf " + debug + " &")
    elif strSecurity() == 'WPA2-PSK':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/ibss_halow_wpa2.conf " + debug + " &")
...
def run_sta(interface):
    ...
    print("[6] Start wpa_supplicant on " + interface)
    if strSecurity() == 'OPEN':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/sta_halow_open.conf " + bridge + debug + " &")
    elif strSecurity() == 'WPA2-PSK':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/sta_halow_wpa2.conf " + bridge + debug + " &")
    elif strSecurity() == 'WPA3-OWE':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/sta_halow_owe.conf " + bridge + debug + " &")
    elif strSecurity() == 'WPA3-SAE':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/sta_halow_sae.conf " + bridge + debug + " &")
    elif strSecurity() == 'WPA-PBC':
        os.system("sudo wpa_supplicant -i" + interface + " -c ${HOME}/nrc_pkg/script/conf/" + country + "/sta_halow_pbc.conf " + bridge + debug + " &")
        time.sleep(1)
...
def run_ap(interface):
    ...
    if(int(self_config)==1 and self_conf_result=='Done'):
        ...
        launch_hostapd( interface, '${HOME}/nrc_pkg/script/conf/' + country + conf_file, country, debug, channel )
        if strSecurity() == 'WPA-PBC':            
            time.sleep(1)
            os.system("sudo hostapd_cli wps_pbc")
    time.sleep(3)
