...
def run_ap():
	...
	os.system("sudo insmod ${HOME}/nrc/nrc.ko hifspeed=16000000 power_save=1 fw_name=nrc7292_cspi.bin")
	time.sleep(5)
	os.system('python ${HOME}/nrc/test/netlink/shell.py run --cmd="phy rxgain 50"')
	time.sleep(1)
	os.system('python ${HOME}/nrc/test/netlink/shell.py run --cmd="phy txgain 1"')
	time.sleep(1)
	os.system('python ${HOME}/nrc/test/netlink/shell.py run --cmd="set capa_1m off on"')
	time.sleep(1)
	os.system("sudo hostapd ${HOME}/nrc/test/ap_halow_ins_rm_test.conf &")
	time.sleep(3)
...
def run_sta():
	...
	time.sleep(1)
	os.system("sshpass -p{0} ssh -o StrictHostKeyChecking=no {1}@{2} 'sudo insmod ${HOME}/nrc/nrc.ko power_save=0 hifspeed=16000000 fw_name=nrc7292_cspi.bin'".format(STA_HOST_PW, STA_HOST_NAME, STA_ETH_IP))
	time.sleep(5)
	os.system("sshpass -p{0} ssh -o StrictHostKeyChecking=no {1}@{2} 'python ${HOME}/nrc/test/netlink/shell.py run --cmd=\"phy rxgain 50\"'".format(STA_HOST_PW, STA_HOST_NAME, STA_ETH_IP))
	time.sleep(1)
	os.system("sshpass -p{0} ssh -o StrictHostKeyChecking=no {1}@{2} 'python ${HOME}/nrc/test/netlink/shell.py run --cmd=\"phy txgain 1\"'".format(STA_HOST_PW, STA_HOST_NAME, STA_ETH_IP))
	time.sleep(1)
	os.system("sshpass -p{0} ssh -o StrictHostKeyChecking=no {1}@{2} 'python ${HOME}/nrc/test/netlink/shell.py run --cmd=\"set capa_1m off on\"'".format(STA_HOST_PW, STA_HOST_NAME, STA_ETH_IP))
	time.sleep(1)
	os.system("sshpass -p{0} ssh -o StrictHostKeyChecking=no {1}@{2} 'sudo wpa_supplicant -Dnl80211 -iwlan0 -c ${HOME}/nrc/test/sta_halow_ins_rm_test.conf &' &".format(STA_HOST_PW, STA_HOST_NAME, STA_ETH_IP))
