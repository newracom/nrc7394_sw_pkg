...
def sshpass(name, type_, option, disp=False, err=False):
	...
	return terminal("/usr/bin/sshpass -p 'antjslan' ssh pi@nrc-halow-{} python ${HOME}/{}-interface/config_{}.py {}{}".format(name, type_, type_, option, err_redirect), disp)

# Reset
def reset(name, type_, retry=10):
	res = terminal("python ${HOME}/hub-interface/hub.py --reset="+name, True)
	option = "--run test"
	if res == 0:
		time.sleep(10)
		while (retry > 0):
